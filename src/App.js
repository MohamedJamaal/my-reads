...
function App(props) {
  ...
  const selectShelf = (selectedBook, shelf) => {
    BooksAPI.update(selectedBook, shelf).then((response) => {
      selectedBook.shelf = shelf;
      // console.log(books);

      setBooks(
        books.filter((book) => book.id !== selectedBook.id).concat(selectedBook)
      );
    });
  };
  ...
  return (
    <div className="app">
      <Routes>
        <Route
          path="/"
          element={<HomePage books={books} selectShelf={selectShelf} />}
        />

        <Route
          path="/search"
          element={<SearchPages books={books} selectShelf={selectShelf} />}
        />
      </Routes>
    </div>
  );
